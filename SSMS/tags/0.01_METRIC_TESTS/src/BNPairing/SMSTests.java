...
public class SMSTests {
    ...
    public static void benchmarks(int BM) {
        ...
        */
        ...
        long elapsed;
        for (int i = SMSParams.nextValid(0); i != 0; i = SMSParams.nextValid(i)) {
	...
	public static void BDCPS(int i, int BM) {
		...
		////////////////////////////////////////////////////////////////////
        Output.println("Benchmarking private RSA-" + 4*sms.p.bitLength());
        BigInteger p = BigInteger.probablePrime(2*sms.p.bitLength(), rnd);
        BigInteger q = BigInteger.probablePrime(2*sms.p.bitLength(), rnd);
        BigInteger u = q.modInverse(p);
        BigInteger n = p.multiply(q);
		...
		////////////////////////////////////////////////////////////////////
        Output.println("Benchmarking public RSA-" + 4*sms.p.bitLength());
        elapsed = -System.currentTimeMillis();
        for (int t = 0; t < BM; t++) {
            m = m.modPow(e, n);
        }
