...
O grupo é finito se $\G$ é um conjunto finito, no caso em que o número de elementos em $_G$ é chamado a ordem de G.
...
%TODO arrumar
Identity-based cryptography became feasible when instantiated with the help of bilinear maps, or \emph{pairings} for short~\cite{sakai-ohgishi-kasahara,boneh-franklin}, and has since attracted widespread attention due to its unusual properties. Pairings are formally defined as follows. Let $k$ be a security parameter and $n$ be a $k$-bit prime number. Consider groups $\G_1$, $\G_2$ and $\G_T$ of order $n$. We say that $(\G_1,\G_2,\G_T)$ are pairing groups if there exists a bilinear map (i.e. a pairing) $e: \G_1 \times \G_2 \rightarrow \G_T$ satisfying the following properties:
\begin{enumerate}
\item Bilinearity: $\forall (S,T) \in \G_1 \times \G_2$, $\forall a,b \in \Z_n$, $e(aS, bT) = e(S,T)^{ab}$.
\item Non-degeneracy: $\forall S \in \G_1$, $e(S,T)=1$ for all $T \in \G_2$ iff $S=\mathcal{O}_{\G_1}$.
\item Computability: $\forall (S,T) \in \G_1 \times \G_2$, $e(S,T)$ is efficiently computable.
\end{enumerate}
