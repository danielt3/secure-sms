...
Aos nossos professores, por guiarnos na busca pelo conhecimento necessário à realização do trabalho.
...
\begin{resumo}
Este trabalho consiste na especificação e implementação de um sistema que garanta serviços de segurança na troca de mensagens SMS entre aperelhos de telefonia celular. Ao longo da monografia, apresentaremos aspectos da nossa solução, métricas, um esquema criptográfico inovador que viabilizou a implementação, especificação do sistema e resultados de testes de desempenho do sistema implementado.
\end{resumo}

\begin{abstract}
This work consists in the specification and implementation of a system that guarantees security services while exchanging SMS between mobile phone devices. Throughout this monograph, we'll present aspects of our solution, metrics, an innovative cryptographic scheme that made the implementation possible, specification of the system and performance test results.
\end{abstract}
...
\item Nível de segurança: Desejamos que o nível de segurança de nosso protocolo seja equiparável ao nível de segurança do RSA usando chaves de 1024 bits, que pode ser calculado como $2^{1024}$ \cite{rsalabs}.
...
Dado que o tempo de uso do RSA de 1024 bits está no fim, uma nova versão será necessária \cite{rsalabs}. Contudo, para um aumento no nível de segurança do RSA, é preciso aumentar consideravelmente o tamanho das chaves, uma vez que a relação entre o tamanho das chaves e o nível de segurança é exponencial, a chave cresce muito rapidamente quando aumenta-se o nível de segurança desejado. %TODO TODO TODO: colocar a fórmula, é exponencial? citar ref...

Em paralelo, um aumento equivalente no nível de segurança de criptografia em curvas elípticas acarreta menor aumento no tamanho das chaves. Este fato ocorre devido à relação entre o nível de segurança de criptografia em curvas elípticas e o tamanho das chaves, que é uma relação diretamente proporcional (cresce linearmente). Para uma chave $k$, tem-se um nível de seguraça de aproximadamente $k/2$. %TODO: citar algo! Colocar a fórmula... é 2^k/2 ? Acho que isso é só uma aproximação... Citar ref..
...
\begin{figure}[h]
	\centering
		\includegraphics[width=1.00\textwidth]{figuras/SequenceDiagramKGB.png}
...
\begin{figure}[h]
	\centering
		\includegraphics[width=1.00\textwidth]{figuras/SequenceDiagramClient.png}
...
Além disto, o nível de segurança desejado também foi alcançado. Com uma chave de 176 bits, obtivemos um nível de segurança equivalente ao do RSA com 1024 bits, conforme previsto.
