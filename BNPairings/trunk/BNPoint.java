...
public class BNPoint {
    ...
    public BNPoint add(BNPoint Q) {
        ...
        */
        ...
        BigInteger p = E.bn.p; // shorthand
        if (Q.z.compareTo(_1) == 0 && z.compareTo(_1) != 0) {
        	BigInteger a, b, c, d, e, f, g, h, i;
        	a = z.multiply(z).mod(p);
        	b = z.multiply(a).mod(p);
        	c = Q.x.multiply(a).mod(p);
        	d = Q.y.multiply(b).mod(p);
        	e = c.subtract(x).mod(p);
        	f = d.subtract(y).mod(p); // lambda numerator!
        	g = e.multiply(e).mod(p);
        	h = g.multiply(e).mod(p);
        	i = x.multiply(g).mod(p);
        	a = f.multiply(f).subtract(h.add(i.shiftLeft(1))).mod(p);
        	b = f.multiply(i.subtract(a)).subtract(y.multiply(h)).mod(p);
        	c = z.multiply(e).mod(p);
        	return new BNPoint(E, a, b, c, f);
        } else {
